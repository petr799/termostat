...
#SELECT * FROM `programy` WHERE den = DAYOFWEEK(CURDATE());  
  cur.execute("SELECT konecna_teplota FROM prog_room1 WHERE (start_time <= CURTIME()) and (end_time >= CURTIME()) and (den = DAYOFWEEK(CURDATE()))")
  endtemp[0] = cur.fetchone()

  print "konecna room1 : %s " % endtemp[0]
